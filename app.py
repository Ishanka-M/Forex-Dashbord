...
# --- 1. SETUP & STYLE ---
# පද්ධතියේ නම v6.0 ලෙස යාවත්කාලීන කරන ලදී
...
<style>
    ...
    .entry-box { background: rgba(0, 212, 255, 0.07); border: 2px solid #00d4ff; padding: 20px; border-radius: 12px; margin-top: 10px; box-shadow: 0 4px 15px rgba(0,212,255,0.1); }
...
def get_ai_analysis(prompt, asset_data):
    # 1. Gemini 3 Flash Preview (අලුත්ම තාක්ෂණය)
    if "GEMINI_KEYS" in st.secrets:
        for i, key in enumerate(st.secrets["GEMINI_KEYS"]):
            try:
                genai.configure(api_key=key)
                # 2026 Release Note අනුව gemini-3-flash-preview භාවිතා කරයි
        ...
        with c_ai:
            ...
            if st.button("Generate Gemini 3 Insight", use_container_width=True):
                with st.spinner("Gemini 3 Flash analyzing market structure..."):
                    ...
                    """
                    st.session_state.ai_result, st.session_state.active_provider = get_ai_analysis(prompt, {'price': curr_p})
                    st.toast(f"Powered by: {st.session_state.active_provider}")
